...
void DebugPass::DrawDebug(nvrhi::BufferHandle instanceBuffer, size_t instanceCount, nvrhi::BufferHandle LineBuffer, size_t LineCount, const ragdoll::SceneInformation& sceneInfo, ragdoll::SceneRenderTargets* targets)
{
	RD_SCOPE(Render, Debug);
	RD_GPU_SCOPE("Debug", CommandListRef);
	DrawBoundingBoxes(instanceBuffer, instanceCount, sceneInfo, targets);
	DrawLines(LineBuffer, LineCount, sceneInfo, targets);
}
...
void DebugPass::DrawLines(nvrhi::BufferHandle LineBuffer, size_t LineCount, const ragdoll::SceneInformation& sceneInfo, ragdoll::SceneRenderTargets* targets)
{
	if (LineCount == 0)
		return;
	//create a constant buffer here
	...
	bindingSetDesc.bindings = {
		nvrhi::BindingSetItem::ConstantBuffer(0, ConstantBufferHandle),
		nvrhi::BindingSetItem::StructuredBuffer_SRV(0, LineBuffer),
	};
	nvrhi::BindingLayoutHandle BindingLayoutHandle = AssetManager::GetInstance()->GetBindingLayout(bindingSetDesc);
	nvrhi::BindingSetHandle BindingSetHandle = DirectXDevice::GetInstance()->CreateBindingSet(bindingSetDesc, BindingLayoutHandle);
	...
	nvrhi::GraphicsPipelineDesc PipelineDesc;
	PipelineDesc.addBindingLayout(BindingLayoutHandle);
	nvrhi::ShaderHandle VertexShader = AssetManager::GetInstance()->GetShader("DebugLineShader.vs.cso");
	nvrhi::ShaderHandle PixelShader = AssetManager::GetInstance()->GetShader("DebugLineShader.ps.cso");
	PipelineDesc.setVertexShader(VertexShader);
	PipelineDesc.setFragmentShader(PixelShader);
	...
	CommandListRef->beginMarker("Debug Line Draws");
	CommandListRef->writeBuffer(ConstantBufferHandle, &CBuffer, sizeof(CBuffer));
	CommandListRef->setGraphicsState(state);

	nvrhi::DrawArguments args;
	args.vertexCount = LineCount;
